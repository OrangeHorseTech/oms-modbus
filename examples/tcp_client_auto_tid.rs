...
//!
//! # TCP Client Example (Auto-increment TID)
//!
//! Demonstrates a TCP client with auto-incrementing Transaction ID.
//! Each request advances the TID counter (1, 2, 3, …), which aids
//! request/response matching in high-throughput scenarios.
//! Uses BusCapture for in-memory packet capture — compare the TID
//! values across captured frames with the standard TID=0 example.
//!
//! ## Usage
//!
//! ```bash
//! cargo run --example tcp_client_auto_tid
//! ```
//!
//! Requires: Modbus TCP device (or simulator) at the configured address.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("═══ OMS Modbus — TCP Client (Auto TID) ═══\n");
    println!("Protocol: TCP — MBAP header + PDU");
    println!("MBAP config: TID=auto (increments per request)");
    ...
    // 2. Connect to server with auto-increment TID config
    println!("[2/3] Connecting to {addr} …");
    let cfg = tcp::TcpConfig {
        tid: tcp::TidMode::Auto,
    ...
    let client = tcp::with_options(addr, opts)
        ...
        .with_config(cfg);
    println!("   ✓ Connected (TID auto-increment)\n");
