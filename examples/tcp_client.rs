...
//!
//! # TCP Client Example (Standard MBAP)
//!
//! Demonstrates a TCP client with standard MBAP framing (TID=0, standard
//! length, no UID in body). Uses BusCapture for in-memory packet capture.
//! Loops through FC01–FC04, printing results in real time, followed by a
//! capture summary.
//!
//! ## Usage
//!
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("═══ OMS Modbus — TCP Client (Standard MBAP) ═══\n");
    println!("Protocol: TCP — MBAP header + PDU");
    println!("MBAP config: TID=0 (fixed), standard length, no UID in body");
    ...
    let client = tcp::with_options(addr, opts).await.map_err(|e| {
        format!(
